    ...
    <form onSubmit={handleSubmit} className="grid gap-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="field-wrap">
          <span>{locale === 'id' ? 'Nama' : 'Name'} *</span>
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder={locale === 'id' ? 'Nama lengkap' : 'Full name'} />
        </label>
        <label className="field-wrap">
          <span>Company *</span>
          <input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} required placeholder={locale === 'id' ? 'Nama perusahaan' : 'Company name'} />
        </label>
        <label className="field-wrap">
          <span>Email *</span>
        ...
        <label className="field-wrap">
          ...
          <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="08..." />
        </label>
      </div>
      <label className="field-wrap">
        <span>{locale === 'id' ? 'Kebutuhan layanan' : 'Service requirement'}</span>
        <input value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })} placeholder="Rotating service / valve maintenance / online leak sealing" />
      </label>
      <label className="field-wrap">
        <span>{locale === 'id' ? 'Detail kebutuhan' : 'Requirement details'} *</span>
